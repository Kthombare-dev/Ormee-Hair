...
export interface VirginProductsPageState {
  currentPage: number;
  search: string;
  searchInput: string;
}
 
const STORAGE_KEY = "virgin-products-page-state";
 
const DEFAULT_STATE: VirginProductsPageState = {
  currentPage: 1,
  search: "",
  searchInput: "",
};
...
const PRODUCTS = Array.from({ length: 12 }, (_, i) => ({
  id: `v${i + 1}`,
  name: `Virgin Hair Product ${i + 1}`,
  price: (i + 1) * 15,
  image: `https://picsum.photos/300?random=virgin-${i + 1}`,
}));
 
const ITEMS_PER_PAGE = 6;
 
export default function VirginHairs() {
  ...
  const updateState = useCallback(
    (newState: Partial<VirginProductsPageState>) => {
      setState((prev) => {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-12">
      <h1 className="text-3xl font-bold mb-2 mt-6">Virgin Hairs Products</h1>
      <p className="text-gray-500 mb-8">
        Explore our premium virgin hair product showcase
      </p>
